...
public:
	...
	*/
	...
	// Returns every character intersecting the line. Battlefield's moving doors
	// use the projected core position to keep characters on the correct side.
	// PureQuery is used by smoke, where moving every character in a 300-pixel
	// radius to the one-pixel query line is the teleport bug.
	std::list<class CCharacter *> IntersectedCharacters(vec2 Pos0, vec2 Pos1, float Radius, class CEntity *pNotThis = 0, bool PureQuery = false);
