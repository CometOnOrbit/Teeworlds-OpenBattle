...
def get_objs():
	# OpenBattle NetObject has no DDNet `ex`; filter only if present
	return ["NETOBJ_INVALID"] + [
		m.enum_name for m in network.Objects
		if getattr(m, "ex", None) is None
	]

def get_objs_7():
	return ["NETOBJ_INVALID"] + [
		m.enum_name for m in seven.network.Objects
		if getattr(m, "ex", None) is None
	]
