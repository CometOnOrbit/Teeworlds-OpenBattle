...
CEntity::~CEntity()
{
	...
	for(int i = 0; i < m_NumExtraIDs; i++)
		Server()->SnapFreeID(m_aExtraIDs[i]);
}

void CEntity::AllocExtraIDs(int NumIDs)
{
	dbg_assert(NumIDs >= 0 && NumIDs <= 9, "invalid number of entity snapshot IDs");
	for(int i = m_NumExtraIDs; i < NumIDs; i++)
		m_aExtraIDs[i] = Server()->SnapNewID();
	m_NumExtraIDs = NumIDs;
}
