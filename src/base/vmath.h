...
{
	// Keep intermediate length math in long double so hooks stay pixel-stable
	// across native x86-64 builds (sqrtf-first can shift velocity by one unit).
	long double X = (long double)v.x;
	long double Y = (long double)v.y;
	long double InvLength = 1.0L/sqrtl(X*X + Y*Y);
	return vector2_base<T>((T)(X*InvLength), (T)(Y*InvLength));
}
