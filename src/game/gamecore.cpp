...
static long double VelocityRampExtended(float Value, float Start, float Range, float Curvature, float *pStoredRamp)
{
	if(Value < Start)
	{
		*pStoredRamp = 1.0f;
	...
	// powf gets a float exponent; the reciprocal stays in long double for the
	// first velocity multiply. Move also stores a float copy for the inverse.
	float Exponent = (float)(((long double)Value-(long double)Start)/(long double)Range);
	float Power = powf(Curvature, Exponent);
	long double Ramp = 1.0L/(long double)Power;
	*pStoredRamp = (float)Ramp;
	return Ramp;
}
...
void CCharacterCore::Tick(bool UseInput)
{
	...
	if(m_HookState == HOOK_GRABBED)
	{
		...
		{
			vec2 HookDirection = normalize(m_HookPos-m_Pos);
			// Keep each direction*acceleration*drag intermediate in long double
			// and round only when HookVel is stored, so native x86-64
			// builds preserve that observable snapshot rounding.
			long double HookDragAccel = (long double)m_pWorld->m_Tuning.m_HookDragAccel;
			vec2 HookVel;
			// the hook as more power to drag you up then down.
			// this makes it easier to get on top of an platform
			long double HookVelY = (long double)HookDirection.y*HookDragAccel;
			if(HookVelY > 0)
				HookVelY *= (long double)0.3f;
			HookVel.y = (float)HookVelY;

			// the hook will boost it's power if the player wants to move
			// in that direction. otherwise it will dampen everything abit
			long double HookVelX = (long double)HookDirection.x*HookDragAccel;
			if((HookVelX < 0 && m_Direction < 0) || (HookVelX > 0 && m_Direction > 0))
				HookVelX *= (long double)0.95f;
			else
				HookVelX *= (long double)0.75f;
			HookVel.x = (float)HookVelX;
...
{
	long double VelX = (long double)m_Vel.x;
	long double VelY = (long double)m_Vel.y;
	float Speed = (float)(sqrtl(VelX*VelX + VelY*VelY)*(long double)50.0f);
	float RampValue;
	long double RampExtended = VelocityRampExtended(Speed, m_pWorld->m_Tuning.m_VelrampStart,
		m_pWorld->m_Tuning.m_VelrampRange, m_pWorld->m_Tuning.m_VelrampCurvature, &RampValue);

	m_Vel.x = (float)((long double)m_Vel.x*RampExtended);

	vec2 NewPos = m_Pos;
	m_pCollision->MoveBox(&NewPos, &m_Vel, vec2(28.0f, 28.0f), 0);

	m_Vel.x = (float)((long double)m_Vel.x*(1.0L/(long double)RampValue));
