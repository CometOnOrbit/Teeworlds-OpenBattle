...
public:
	...
	CGameWorld m_World;
	bool m_BattlefieldWaterEnabled;

	// Battlefield's three capturable checkpoints. Negative progress belongs to
	// red, positive progress to blue; the endpoints are -400 and +400.
	int m_aCheckpointState[3];
	bool m_aCheckpointCaptured[3];
	bool m_aCheckpointWarning[3][2];
	int m_aCheckpointWarningCooldown[2];
	// Gate checkpoint progress cues independently of the warning cooldowns.
	int m_aCheckpointProgressSoundCooldown[3];
	vec2 m_aCheckpointDestination[3];
	int m_aDoorState[6];
	vec2 m_aDoorStart[6];
	vec2 m_aDoorEnd[6];
	...
	// Battlefield entities treated each active moving door as a
	// thick line and sampled their complete movement path against it.
	bool IntersectBattlefieldDoor(vec2 From, vec2 To, vec2 *pHit=0, float Radius=30.0f);
	...
	void CreateDamageIndd(vec2 Pos, float AngleMod, int Amount);
	void CreateExplosion(vec2 Pos, int Owner, int Weapon, bool NoDamage);
	void CreateExplosion2(vec2 Pos, int Owner, int Weapon, int Damage);
